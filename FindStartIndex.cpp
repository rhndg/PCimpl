...
const GLchar* find_start_vert_shader = GLSL(
    ...
    int cell_index(int extended_index){
        ...
        vec3 position = textureLod(position_arr, decode_particle_index(extended_index), 0).rgb;
    ...
    void main(){
    	int first_cell_index = cell_index(textureLod(index_arr, decode_particle_index(0)              , 0).r);
    	int last_cell_index  = cell_index(textureLod(index_arr, decode_particle_index(n_particles - 1), 0).r);
		...
		if(my_cell_index > first_cell_index 
		  && my_cell_index <= last_cell_index){
	    	...
	    	while(true){
	    		mid = (high + low) / 2;
	    		int mid_extended_index = textureLod(index_arr, decode_particle_index(mid), 0).r;
